...
RSpec.describe SiteSettings::TypeSupervisor do
  ...
  describe ".type_hash" do
    ...
    it "returns accepted_extensions for upload type" do
      settings.setting(
      ...
      expect(hash[:accepted_extensions]).to eq ".txt"
    end

    it "does not return accepted_extensions when not specified" do
      settings.setting(:type_upload_no_extensions, "", type: "upload")
      settings.refresh!

      hash = settings.type_supervisor.type_hash(:type_upload_no_extensions)
      expect(hash[:type]).to eq "upload"
      expect(hash[:accepted_extensions]).to be_nil
    end
