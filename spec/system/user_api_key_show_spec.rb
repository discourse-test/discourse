...
RSpec.describe "User API Key Show Page", type: :system do
  ...
  it "displays the copy button and copies the payload without whitespace" do
    ...
    # The displayed payload should have whitespace (newlines from Base64.encode64)
    expect(displayed_payload).to include("\n")
    ...
    expect(clipboard_content).to eq(displayed_payload.gsub(/\s/, ""))
    expect(clipboard_content).not_to include("\n")
    expect(clipboard_content).not_to include(" ")
