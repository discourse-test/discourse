...
class UploadValidator < ActiveModel::Validator
  ...
  def validate_site_setting_upload(upload, extension)
    ...
    accepted_extensions = setting_opts[:accepted_extensions]

    if accepted_extensions.present?
      unless extension_allowed_for_site_setting?(extension, accepted_extensions)
        upload.errors.add(
          :original_filename,
          I18n.t("upload.unauthorized", authorized_extensions: accepted_extensions),
        )
        return false
      end
